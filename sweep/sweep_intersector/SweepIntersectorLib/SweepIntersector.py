...
class SweepIntersector():
    ...
    def _checkAndReturnIntersection(self, sitPred):
        """
        Helper method to check for intersections and return if found.
        """
        xit = self.Y_structure.inf(sitPred)

        if xit is not None:
            s1 = self.Y_structure.key(sitPred)
            s2 = self.Y_structure.key(self.Y_structure.succ(sitPred))
            self.interDic[(s1.id, s2.id)] = xit
            self.Y_structure.changeInf(sitPred, None)

            # Check for intersections and return if found
            if self.computeIntersection(sitPred):
                return True
